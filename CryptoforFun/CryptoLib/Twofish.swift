...
public final class Twofish {
    ...
    public enum Variant: Int {
        case twofish128, twofish192, twofish256
    ...
    private static let RS: Array<UInt8> = [
    ...
    private static let q0: Array<UInt8> = [
    ...
    private static let q1: Array<UInt8> = [
    ...
    private static let MDS: Array<Array<UInt32>> = [
...
private extension Twofish {
    private func mult(_ lhs: UInt8, _ rhs: UInt8) -> UInt8 {
        var a = lhs, b = rhs
    ...
    private func H(_ X: UInt32, _ L: Array<UInt32>) -> UInt32 {
        ...
        let l = Array(L.map { Array($0.bytes()[4..<8].reversed()) })
        if (L.count == 4) {
            x[0] = Twofish.q1[Int(x[0])] ^ l[3][0]
            x[1] = Twofish.q0[Int(x[1])] ^ l[3][1]
            x[2] = Twofish.q0[Int(x[2])] ^ l[3][2]
            x[3] = Twofish.q1[Int(x[3])] ^ l[3][3]
        }
        
        if (L.count >= 3) {
            x[0] = Twofish.q1[Int(x[0])] ^ l[2][0]
            x[1] = Twofish.q1[Int(x[1])] ^ l[2][1]
            x[2] = Twofish.q0[Int(x[2])] ^ l[2][2]
            x[3] = Twofish.q0[Int(x[3])] ^ l[2][3]
        }
        
        x[0] = Twofish.q0[Int(x[0])] ^ l[1][0]
        x[1] = Twofish.q1[Int(x[1])] ^ l[1][1]
        x[2] = Twofish.q0[Int(x[2])] ^ l[1][2]
        x[3] = Twofish.q1[Int(x[3])] ^ l[1][3]
        
        x[0] = Twofish.q0[Int(x[0])] ^ l[0][0]
        x[1] = Twofish.q0[Int(x[1])] ^ l[0][1]
        x[2] = Twofish.q1[Int(x[2])] ^ l[0][2]
        x[3] = Twofish.q1[Int(x[3])] ^ l[0][3]
        
        x[0] = Twofish.q1[Int(x[0])]
        x[1] = Twofish.q0[Int(x[1])]
        x[2] = Twofish.q1[Int(x[2])]
        x[3] = Twofish.q0[Int(x[3])]
        
        let z0 = Twofish.MDS[0][Int(x[0])]
        let z1 = Twofish.MDS[1][Int(x[1])]
        let z2 = Twofish.MDS[2][Int(x[2])]
        let z3 = Twofish.MDS[3][Int(x[3])]
        
        return z0 ^ z1 ^ z2 ^ z3
    }
    
    private func expandKeyAndSBox(key: Key) -> (Array<UInt32>, Array<UInt32>) {
        let keyBytes = key.bytes
        let kLength = variantNr * 2 + 8
        let k = UnsafeMutablePointer<UInt32>.allocate(capacity: kLength)
        k.initialize(to: 0, count: kLength)
        defer {
            k.deinitialize(count: kLength)
            k.deallocate(capacity: kLength)
        }
        
        let sLength: Int = key.count / 8
        ...
        for i in 0..<(variantNk / 2) {
            let s = Twofish.RS.enumerated()
                .map { (idx, element) in (element, keyBytes[i * 8 + idx % 8]) }
                .map { (rs, subkey) in mult(rs, subkey) }.batched(by: 8)
                .map { $0.reduce(0) { $0 ^ $1 } }
            S[variantNk / 2 - i - 1] = UInt32(bytes: s, endian: .littleEndian)
        }
        
        let M = keyBytes.batched(by: 4).map { UInt32(bytes: $0, endian: .littleEndian) }.batched(by: 2)
        let Me = M.map { $0[0] }, Mo = M.map { $0[1] }
